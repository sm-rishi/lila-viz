...
const COMPARE_SIZE = Math.floor((CANVAS_BASE - 20) / 2); // 400px each
...
export default function MapView({
  ...
  // All-layers data for POI metrics + grid hover
  const needAllLayers = (mode === 'heatmap' || mode === 'compare') && mapId;
  const { data: allHeatmapsData } = useAllHeatmaps(
    needAllLayers ? mapId : null,
    filterDay
  );
  ...
  const handleMouseDown = useCallback((e) => {
    // If POI tool is active, start drawing a zone instead of panning
    if (poiTool) {
  ...
  }, [zoom, pan, poiTool]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [zoom, pan, poiTool, poiDrawStart, pois.length]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    // Update POI drawing preview
    if (poiTool && poiDrawStart) {
      setPoiDrawCurrent({ x: cx, y: cy });
    }

    // Grid hover for heatmap mode
    if ((mode === 'heatmap') && showGrid) {
      const CELL_PX = CANVAS_BASE / (1024 / 16); // = CANVAS_BASE * 16/1024
      const bx = Math.floor(cx / CELL_PX);
      const by = Math.floor(cy / CELL_PX);
      if (bx >= 0 && bx < 64 && by >= 0 && by < 64) {
        setHoveredCell([bx, by]);
        const cellMetrics = getGridCellMetrics(bx, by, allHeatmapsData);
        setTooltip(cellMetrics ? { type: 'grid', x: sx, y: sy, bx, by, metrics: cellMetrics } : null);
      } else {
        setHoveredCell(null);
        setTooltip(null);
      }
  ...
  }, [events, currentT, mode, zoom, pan, handleMouseMovePan, showGrid, allHeatmapsData,
      poiTool, poiDrawStart]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const drawingShape = poiDrawStart && poiDrawCurrent ? {
    type: poiTool,
    startX: poiDrawStart.x,  startY: poiDrawStart.y,
    endX:   poiDrawCurrent.x, endY:  poiDrawCurrent.y,
  } : null;

  // ── Feature 4 — Grid Metrics ───────────────────────────────────────────────
  ...
  // Diff computation for compare mode
  const diffData = useMemo(() => {
    ...
    allKeys.forEach(key => {
      const lv    = ((leftData.cells[key]  || 0) / lMax);
      const rv    = ((rightData.cells[key] || 0) / rMax);
      const delta = rv - lv;
  ...
  // Two map images for compare mode (both sides use same map)
  const [compareMapImage, setCompareMapImage] = useState(null);
  useEffect(() => {
    if (!mapId) { setCompareMapImage(null); return; }
    const img = new Image();
    img.src = MINIMAP_IMAGES[mapId];
    img.onload = () => setCompareMapImage(img);
  }, [mapId]);
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-950 overflow-auto relative">
        ...
        {mapId && mode === 'compare' && (
          <div className="flex flex-col items-center gap-3 py-4">
            {/* Day selectors + diff toggle toolbar */}
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 bg-gray-900 border border-gray-700 rounded px-3 py-1.5">
                ...
                <select
                  value={compareLeftDay}
                  onChange={e => setCompareLeftDay(e.target.value)}
              ...
              <div className="flex items-center gap-2 bg-gray-900 border border-gray-700 rounded px-3 py-1.5">
                ...
                <select
                  value={compareRightDay}
                  onChange={e => setCompareRightDay(e.target.value)}
                  className="bg-transparent text-gray-200 text-xs border-none outline-none pr-1"
                >
                  <option value="">All Dates</option>
                  {DAYS.map(d => <option key={d} value={d}>{DAY_LABELS[d]}</option>)}
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <div className="relative shadow-2xl">
                  <HeatmapCanvas
                    heatmapData={compareDiff ? leftData  : leftData}
                    diffData={compareDiff ? null : null}
                    mapImage={compareMapImage}
                    canvasSize={COMPARE_SIZE}
              ...
              <div>
                ...
                <div className="relative shadow-2xl">
                  <HeatmapCanvas
                    heatmapData={rightData}
                    diffData={compareDiff ? diffData : null}
                    mapImage={compareMapImage}
                    canvasSize={COMPARE_SIZE}
                    layerKey={heatmapLayer}
                    opacity={hmOpacity}
            ...
            {compareDiff && (
              <div className="flex items-center gap-3 text-xs text-gray-400 bg-gray-900 border border-gray-700 rounded px-4 py-1.5">
                <div className="w-3 h-3 rounded-sm bg-blue-400 opacity-80" />
                <span>More in A</span>
        ...
        {mapId && mode !== 'compare' && (
          <div
            ...
            onMouseLeave={() => { dragging.current = false; setTooltip(null); setHoveredCell(null); setPoiDrawCurrent(null); }}
          >
            {/* Inner zoom/pan transform div */}
            <div
              style={{
                width: CANVAS_BASE, height: CANVAS_BASE,
            ...
            >
              ...
              {/* POI shapes SVG (inside zoom div so they scale with canvas) */}
              {(mode === 'heatmap') && (
                <POIOverlay
                  canvasSize={CANVAS_BASE}
                  zoom={zoom}
            ...
            {/* Tooltip (in screen space, outside zoom) */}
            {tooltip?.type === 'event' && (
              <div
            ...
            {tooltip?.type === 'grid' && (
              <div
                className="absolute z-20 bg-gray-900 border border-gray-600 rounded px-2 py-1.5 text-xs pointer-events-none shadow-lg"
                style={{ left: Math.min(tooltip.x + 12, CANVAS_BASE - 130), top: Math.min(tooltip.y - 8, CANVAS_BASE - 100) }}
              >
        ...
        {/* POI metrics panel — outside zoom div so it doesn't transform */}
        {mode === 'heatmap' && mapId && (
          <POIMetricsPanel
            pois={pois}
            setPois={setPois}
        ...
        {/* ── Heatmap tools (opacity + grid + POI) ───────────────────── */}
        {mode === 'heatmap' && mapId && (
          <div className="absolute bottom-4 right-4 flex flex-col gap-2 items-end">
            ...
            <div className="bg-gray-900 border border-gray-700 rounded px-2 py-1.5 flex gap-1.5 items-center">
              ...
              {[
                ...
                { key: 'rect',   label: '▭', title: 'Draw rect zone'   },
              ].map(({ key, label, title }) => (
                <button
                  key={key}
                  onClick={() => setPoiTool(prev => prev === key ? null : key)}
                  title={title}
        ...
        {selectedMatch && selectedMatch.match_id && mode !== 'compare' && (
